...
class TranslatorApp(ctk.CTk):
    ...
    def _build_ui(self) -> None:
        ...
        self.var_ai_batch = ctk.IntVar(value=int(settings.get("GENERAL", "batch_size") or 40))
        self.lbl_ai_batch = ctk.CTkLabel(
            self.frame_ai,
            text=f"Строк в пакете: {self.var_ai_batch.get()}",
            font=("", 11, "bold"),
        )
        self.lbl_ai_batch.pack(anchor="w", pady=(8, 0))
        ctk.CTkSlider(
            self.frame_ai,
            from_=5,
            to=100,
            number_of_steps=19,
            variable=self.var_ai_batch,
            command=lambda v: self.lbl_ai_batch.configure(text=f"Строк в пакете: {int(v)}"),
    ...
    def _translation_options(self) -> TranslationOptions:
        return TranslationOptions(
            ...
            ai_mode="safe" if self.var_ai_batch.get() < 30 else "context",
